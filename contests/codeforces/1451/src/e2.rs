...
trait FindDuplicatesWithVecBy: Sized {
	fn duplicates_with_vec_by<T: Clone, F: Fn(T) -> usize>(
		self,
		vec_size: usize,
		f: F,
	) -> DuplicatesWithVecBy<Self, F, T>
	where
		Self: std::iter::Iterator<Item = T>;
}
...
impl<I: Iterator> FindDuplicatesWithVecBy for I {
	...
	where
		Self: std::iter::Iterator<Item = T>,
	{
		DuplicatesWithVecBy { vec: vec![None; vec_size], iter: self, f }
	}
}
