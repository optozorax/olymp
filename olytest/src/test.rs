...
where
    P: FnMut(&T) -> bool, T: std::fmt::Debug
{
    type Item = &'a [T];
    ...
    fn next(&mut self) -> Option<&'a [T]> {
        ...
        loop {
	        match self.v.iter().skip(skipped).position(|x| (self.pred)(x)) {
	            ...
	            Some(idx) => {
	            	match self.v.get(skipped + idx+1) {
	            		Some(elem) => {
	            			if (self.pred)(elem) {
	            				skipped += idx + 2;
	            			} else {
...
pub fn read_tests(file: &[u8]) -> Result<Vec<Test>, usize> {
	let mut result = Vec::new();
 	for (test_no, test) in file.split_not_double(|x| *x == b'\\').enumerate() {
 		...
 		let second = (iter_checker.next(), iter_checker.next(), iter_checker.next());
 		match (first, second) {
...
pub fn has_checker(tests: &[TokenizedTest<'_>]) -> bool {
	tests.iter().any(|x| matches!(x, TokenizedTest::WithChecker { .. }))
}
...
mod tests {
	...
	fn split_not_double() {
		...
		let collected =  b"0~~1~~~~2~~~a~~z".split_not_double(|x| *x == b'~').collect::<Vec<_>>();
		...
		let collected =  b"0~1~2".split_not_double(|x| *x == b'~').collect::<Vec<_>>();
		assert_eq!(should_be, collected);
	...
	#[test]
	fn read() {
		let a = read_tests(b"1~\\2%\\0%%1 2 3 4~~5 ~3\\5 6 7%\n  \n   \r\t   \\1%abc def      xyz\n1\n\r  2\t3  ").unwrap();
		assert_eq!(a, vec![
			Test::SimpleInput {
		...
		assert_eq!(b, vec![
			...
			TokenizedTest::WithChecker {
				checker_input: b"1",
				checker_output: Some(Lines(vec![Tokens(vec![b"abc", b"def", b"xyz"]), Tokens(vec![b"1"]), Tokens(vec![b"2", b"3"])])),
			},
		]);
	}
}